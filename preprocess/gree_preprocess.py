...
import tf_extended as tf_ext
import preprocess.tf_image as tf_image
...
def distorted_bounding_box_crop(image,
  ...
  with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bboxes]):
    # Each bounding box has shape [1, num_boxes, box coords(i.e. 4)] and
    # the coordinates are ordered [ymin, xmin, ymax, xmax].

    # get a single bounding box that may be used to crop the original image.
    ...
    # Resize: Use distort_box's(i.e. Crop Image) y_min , x_min as origin, height / weight as a unit length
    bboxes = tf_ext.bboxes_resize(distort_bbox, bboxes)
    # Filter: For those in bboxes, filter out those overlap < BBOX_CROP_OVERLAP boxes
    labels, bboxes = tf_ext.bboxes_filter_overlap(labels, bboxes,
...
def preprocess_for_train(image, labels, bboxes,
                         ...
                         scope='gree_preprocessing_train'):
  """
  ...
  """
  fast_mode = False
  with tf.name_scope(scope, 'gree_preprocessing_train', [image, labels, bboxes]):
    if image.get_shape().ndims != 3:
      raise ValueError('Input must be of size [height, width, C>0]')
    ...
    dst_image = image
    # generate a image crop and corresponding bboxes in relative style, i.e. in range [0,1].
